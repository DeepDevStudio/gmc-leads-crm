...
import { sendBulkMessages } from "../services/bulkService";
  ...
  const handleSend = async () => {
    try {
      ...
      await createActivity({
        ...
        activity: `Sent ${response.total} WhatsApp Messages`,
      });

      alert(`${response.total} Messages Sent`);
    } catch (error) {
      console.error(error);
      alert("Send Failed");
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500">Match Customers With Templates</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow border p-6">
        ...
        <button
          ...
          className="bg-blue-500 hover:bg-blue-600 text-white px-8 py-4 rounded-xl font-semibold ml-4"
        >
      ...
      <div className="bg-white rounded-2xl shadow border overflow-hidden">
        <div className="p-4 border-b">
          <h2 className="font-bold">Total Matches: {results.length}</h2>
        </div>
